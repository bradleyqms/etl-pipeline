/* EXPORT: FACT_SALES_US_YTD
   Logic: Historical YTD export for USA with CSV text qualifiers.
*/

-- ==========================================================
-- UNITED STATES (US - QMSUSA) - Invoices & Invoiced Credit Notes
-- ==========================================================
SELECT 
    CHAR(34) + 'US' + CHAR(34) AS "Entity", 
    T0."DocEntry", 
    T0."DocNum", 
    T0."DocDate",
    ...
    CHAR(34) + CAST(T1."ItemCode" AS NVARCHAR(50)) + CHAR(34) AS "ItemCode", 
    ...
    T1."SlpCode", 
    T0."UpdateDate"
FROM [A20180_QMSUSA_P01].[dbo].[OINV] T0 
INNER JOIN [A20180_QMSUSA_P01].[dbo].[INV1] T1 ON T0."DocEntry" = T1."DocEntry"
WHERE T0."DocDate" BETWEEN '2026-01-01' AND CAST(GETDATE() AS DATE)
  AND T0."CANCELED" = 'N'

UNION ALL

-- ==========================================================
-- UNITED STATES (US - QMSUSA) - Standalone Credit Notes
-- ==========================================================
SELECT 
    CHAR(34) + 'US' + CHAR(34), 
    T0."DocEntry", 
    T0."DocNum", 
    T0."DocDate", 
    CHAR(34) + 'Credit Note' + CHAR(34), 
    T1."LineNum", 
    CHAR(34) + CAST(T0."CardCode" AS NVARCHAR(50)) + CHAR(34), 
    CHAR(34) + CAST(T1."ItemCode" AS NVARCHAR(50)) + CHAR(34), 
    ...
    T1."SlpCode", 
    T0."UpdateDate"
FROM [A20180_QMSUSA_P01].[dbo].[ORIN] T0 
INNER JOIN [A20180_QMSUSA_P01].[dbo].[RIN1] T1 ON T0."DocEntry" = T1."DocEntry"
WHERE T0."DocDate" BETWEEN '2026-01-01' AND CAST(GETDATE() AS DATE)
  AND T0."CANCELED" = 'N';